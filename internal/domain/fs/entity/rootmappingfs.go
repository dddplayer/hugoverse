...
func (m *RootMappingFs) newUnionFile(fis ...valueobject.FileMetaInfo) (afero.File, error) {
	meta := fis[0].Meta()
	f, err := meta.Open()
	if err != nil {
		return nil, err
...
func (m *RootMappingFs) doLstat(name string) ([]valueobject.FileMetaInfo, error) {
	...
	if roots == nil {
		...
		var err error
		var fis []valueobject.FileMetaInfo

		for _, rm := range roots {
			var fi valueobject.FileMetaInfo
	...
	return []valueobject.FileMetaInfo{roots[0].Fi}, nil
}
...
func (m *RootMappingFs) getRoot(key string) []valueobject.RootMapping {
	v, found := m.RootMapToReal.Get(key)
	if !found {
		return nil
	}

	return v.([]valueobject.RootMapping)
}

func (m *RootMappingFs) getRoots(key string) (string, []valueobject.RootMapping) {
	s, v, found := m.RootMapToReal.LongestPrefix(key)
	if !found || (s == paths.FilePathSeparator && key != paths.FilePathSeparator) {
		return "", nil
	}
	return s, v.([]valueobject.RootMapping)
}

func (m *RootMappingFs) statRoot(root valueobject.RootMapping, name string) (valueobject.FileMetaInfo, bool, error) {
	filename := root.Filename(name)

	fmt.Println(">>> RootMappingFs.statRoot", root, name, filename)

	fi, b, err := valueobject.LstatIfPossible(m.Fs, filename) // source fs
	if err != nil {
		return nil, b, err
	}
	...
	return valueobject.DecorateFileInfo(fi, m.Fs, opener, "", "", root.Meta), b, nil
}
...
func (m *RootMappingFs) collectDirEntries(prefix string) ([]os.FileInfo, error) {
	...
	collectDir := func(rm valueobject.RootMapping, fi valueobject.FileMetaInfo) error {
		f, err := fi.Meta().Open()
		if err != nil {
			return err
		}
		...
		for _, fi := range direntries {
			meta := fi.(valueobject.FileMetaInfo).Meta()
			meta.Merge(rm.Meta)
...
func newDirNameOnlyFileInfo(name string, meta *valueobject.FileMeta, fileOpener func() (afero.File, error)) valueobject.FileMetaInfo {
	...
	return valueobject.NewFileMetaInfo(
		valueobject.NewDirNameOnlyFI(base, htime.Now()),
		m,
	)
}
